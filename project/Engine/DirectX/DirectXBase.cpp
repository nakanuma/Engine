...
void DirectXBase::CreateRootSignatureMapchip()
{
	HRESULT result = S_FALSE;
	...
	// バイナリを元に生成
	rootSignatureMapchip_ = nullptr;
	result = device_->CreateRootSignature(0, signatureBlob_->GetBufferPointer(), signatureBlob_->GetBufferSize(), IID_PPV_ARGS(&rootSignatureMapchip_));
	assert(SUCCEEDED(result));
}
...
void DirectXBase::ShaderCompile()
{
	...
	// Mapchip用Shader
	vertexShaderBlobMapchip_ = CompileShader(L"resources/Shaders/Mapchip.VS.hlsl", L"vs_6_0", dxcUtils_, dxcCompiler_, includeHandler_);
	assert(vertexShaderBlob_ != nullptr);

	pixelShaderBlobMapchip_ = CompileShader(L"resources/Shaders/Mapchip.PS.hlsl", L"ps_6_0", dxcUtils_, dxcCompiler_, includeHandler_);
	assert(pixelShaderBlob_ != nullptr);
}
...
void DirectXBase::CreatePipelineStateObject()
{
	...
	// Mapchip用のPSOを作成
	D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipelineStateMapchipDesc = graphicsPipelineStateDefault;
	graphicsPipelineStateMapchipDesc.pRootSignature = rootSignatureMapchip_.Get();
	graphicsPipelineStateMapchipDesc.VS = { vertexShaderBlobMapchip_->GetBufferPointer(), vertexShaderBlobMapchip_->GetBufferSize() };
	graphicsPipelineStateMapchipDesc.PS = { pixelShaderBlobMapchip_->GetBufferPointer(), pixelShaderBlobMapchip_->GetBufferSize() };
	// 生成
	graphicsPipelineStateMapchip_ = nullptr;
	result = device_->CreateGraphicsPipelineState(&graphicsPipelineStateMapchipDesc, IID_PPV_ARGS(&graphicsPipelineStateMapchip_));
}
...
void DirectXBase::BeginFrame()
{
	...
	// 指定した色で画面全体をクリアする
	float clearColor[] = { 0.1f, 0.882f - 0.07f, 0.87f - 0.07f, 1.0f };
